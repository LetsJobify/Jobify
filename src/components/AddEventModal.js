    ...
    >
      ...
      <Button
        ...
        onClick={async () => {
          ...
          let response;
          if (formState.new === true){
            response = await fetch('/interview', request);
            setFormState({ ...formState, refresh: !formState.refresh });}
          else
            response = await fetch(`/interview/${formState.formId}`, request);
            setFormState({ ...formState, refresh: !formState.refresh });
        }}
      >
        Submit
